...
{
    // Área do código
    NoListaSimples *novoNo = (NoListaSimples*)calloc(1,sizeof(NoListaSimples));
    if(novoNo == NULL)
        return NULL;
...
{
    // Área do código
    NoListaSimples *novoNo = CriaNoSimples(valor);
...
void Dilatacao(NoListaSimples **noAtual, int n, int m)
{
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < m; j++) {
            ...
            for (int k = 0; k < 8; k++) {
                if (C[k].x >= 0 && C[k].y >= 0 && C[k].x < n && C[k].y < m) {
                    int indice = C[k].x * m + C[k].y;
                    int valor = ObterNo(*noAtual, indice)->valor;
            ...
            int indice_temp = i * m + j;
            temp[indice_temp].valor = valor_referencia;
        }
    }
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < m; j++) {
            int indice = i * m + j;
            ObterNo(*noAtual, indice)->valor = temp[indice].valor;
        }
    }
